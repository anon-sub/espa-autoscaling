...
def get_total_cpu_cost_from_dataframe(data_frame) -> [int]:
    metric_column = getMetricColumn("taskmanager", data_frame)
    metric_column = interpolate_data_column(metric_column)
    metric_column = metric_column * 15
    results = [0] * 10
    results[1] = np.sum(metric_column)/3600
    return results
