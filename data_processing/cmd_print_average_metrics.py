...
def printAverageData(parameters: SingleFolderPlotParameters, add_metrics_average: bool, add_metrics_percentile_50,
                     add_metrics_percentile_95, add_metrics_maximum: bool, add_total_cpu_cost: bool,
                     custom_metrics_percentile: int = None, postfix_label: str = None):
    ...
    for experiment_file, data_frame in experiment_file_datafile_mapping.items():
        ...
        if add_total_cpu_cost:
            total_cpu_cost = DataProcessor.get_total_cpu_cost_from_dataframe(data_frame)
            total_cpu_cost = round_results(total_cpu_cost)
            total_cpu_row = [experiment_file.get_experiment_name() + "(total_cpu)"] + total_cpu_cost
            table.append(total_cpu_row)
...
def parseArguments():
    ...
    parser.add_argument('--add_total_cpu_cost', action="store_true")
    ...
    add_total_cpu_cost = True if namespace.add_total_cpu_cost else False
    ...
    printAverageData(parameters, add_average_metrics, add_metrics_percentile_50, add_metrics_percentile_95, add_maximum_metrics,
                     add_total_cpu_cost, custom_metrics_percentile=custom_percentile, postfix_label=postfix_label)
