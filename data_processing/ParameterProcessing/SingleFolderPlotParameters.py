...
class SingleFolderPlotParameters(PlotParameters):
    ...
    def get_input_only(self) -> bool:
        """
        Get input_only
        """
        return self.__input_only
    ...
    def include_arguments_in_parser(self, argument_parser: argparse.ArgumentParser):
        ...
        def include_input_only_in_parser(parser: argparse.ArgumentParser):
            parser.add_argument("--input_only", action="store_true",
                                help=f"Flag to print only the input pattern."
                                     f"Default=false")
        ...
        include_input_only_in_parser(argument_parser)
    ...
    def fetch_arguments_from_namespace(self, namespace: argparse.Namespace):
        ...
        def fetch_input_only_from_namespace(args: argparse.Namespace):
            input_only = args.input_only
            # if result_filetype is provided
            if input_only:
                # set result_filetype
                self.__input_only = input_only
        ...
        fetch_input_only_from_namespace(namespace)
